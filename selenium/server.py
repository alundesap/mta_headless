...
def hello_world():
    ...
    output += '<a href="/selenium/links">/selenium/links</a><br />\n'
    return output
...
@app.route('/selenium/links')
def python_links():
    output = '<strong>Python SecureStore! Instance ' + str(os.getenv("CF_INSTANCE_INDEX", 0)) + '</strong> Try these links.</br>\n'
    output += '<a href="/selenium/insert">/selenium/insert</a><br />\n'
    output += '<a href="/selenium/retrieve">/selenium/retrieve</a><br />\n'
    output += '<a href="/selenium/delete">/selenium/delete</a><br />\n'
    return output

@app.route('/selenium/insert')
...
#    # Form an SQL statement to retrieve some data
    ...
    try:
        cursor.callproc("SYS.USER_SECURESTORE_INSERT", ("CredStore", False, "TestKey", hex2store))
        output += 'key TestKey with value ' + string2store + '=' + hex2store + ' was inserted into store CredStore.' + '\n'
    except:
...
#https://blogs.sap.com/2017/07/26/sap-hana-2.0-sps02-new-feature-updated-python-driver/
    ...
    #cursor.execute('call SYS.USER_SECURESTORE_RETRIEVE (\'CredStore\', False, \'TestKey\', ?)')
    hexvalue = cursor.callproc("SYS.USER_SECURESTORE_RETRIEVE", ("CredStore", False, "TestKey", None))
...
#    # Close the DB connection
    ...
    if hexvalue[3] is None:
        output += 'key TestKey does not exist in store CredStore.  Try inserting a value first.' + '\n'
    else:
        retrieved = codecs.decode(hexvalue[3].hex(), "hex").decode()
        output += 'key TestKey with value ' + retrieved + ' was retrieved from store CredStore.' + '\n'

    # Return the results
    return Response(output, mimetype='text/plain' , status=200,)

@app.route('/selenium/delete')
...
#    # Form an SQL statement
    cursor.callproc("SYS.USER_SECURESTORE_DELETE", ("CredStore", False, "TestKey"))

#    # Close the DB connection
    connection.close()
    
    output += 'key TestKey was deleted from store CredStore.' + '\n'
